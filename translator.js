...
timeReplace(text){
  if (/\s([0-1]?[0-2]):([0-5][0-9])/g.test(text) == true){
let replace = text.replace(/\s([0-1]?[0-2]):([0-5][0-9])/g, `<span class="highlight"> $1.$2</span>`) 
return replace
  }
if (/\s([0-1]?[0-2]):([0-5][0-9])/g.test(text) == false){
let replace = text.replace(/\s([0-9]):([0-5][0-9])/g, `<span class="highlight"> $1.$2</span>`)
return replace
}
